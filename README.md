...
**Установщик**: [`Dictation-2.3.1-setup.exe`](https://github.com/metawka/SuperDictate-win/releases/latest)
...
Сочетания не строятся на правом Ctrl: он сам по себе начинает диктовку в
тот момент, когда его нажали, поэтому «Ctrl + что-нибудь» срабатывает
только если второй клавишей нажать Ctrl последним — а руки берут
«Ctrl + Alt» в обратном порядке и получают запись вместо сочетания. У
правого Alt своей функции нет, так что Shift + Правый Alt работает в
любом порядке нажатия. История расшифровок открывается из трея и из
панели управления; отдельного сочетания у неё нет.
...
21 тест на чистой логике: конечный автомат горячих клавиш (включая
проверку, что модификаторы не перехватываются, и что сочетание правки
срабатывает в любом порядке нажатия), обработка текста и стили, разбор
