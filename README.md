...
**Установщик**: [`Dictation-2.4.0-setup.exe`](https://github.com/metawka/SuperDictate-win/releases/latest)
...
| **Shift + Правый Ctrl** | последние расшифровки пузырьками |
| **Shift + Правый Alt** | правка последней расшифровки |
...
Сочетание с правым Ctrl работает, только если модификатор нажат первым:
правый Ctrl сам по себе начинает диктовку в тот момент, когда его нажали.
С Shift это не ограничение — никто не тянется к Shift после клавиши, —
а вот «Ctrl + Alt» руки берут в обратном порядке примерно в половине
случаев, поэтому правка сидит на правом Alt, у которого своей функции
нет, и срабатывает в любом порядке нажатия.
...
- **Последние расшифровки пузырьками** (Shift + Правый Ctrl): капсула
  возвращается на экран, а над ней встаёт стопка из последних шести
  расшифровок — свежая внизу, каждый пузырёк по ширине своего текста.
  Стрелки выбирают, Enter вставляет выбранную в то поле, где вы были,
  Delete удаляет её из истории, Escape закрывает. Полная история со всеми
  записями, копированием и очисткой по-прежнему открывается из трея и из
  панели управления
- **Правка последней расшифровки** (Shift + Правый Alt): капсула
  возвращается на экран, и пузырёк предпросмотра над ней становится полем
  ввода с текстом последней диктовки по центру — курсор, стрелки,
  выделение, клик между букв. Enter применяет, Escape закрывает без
  изменений,
  Shift + Enter переносит строку; фокус возвращается тому окну, из
  которого вызвали. Изменённые слова попадают в словарь замен парами
  «как услышалось» -> «как на самом деле» и применяются к следующим
  диктовкам. Правка только в регистре или в пунктуации не запоминается:
  это не ошибка распознавания. Поле ввода, куда текст уже был вставлен,
  не трогается — Windows не даёт одной программе читать и править текст
  в другой
- **Удаление слов-паразитов** с восстановлением пунктуации и заглавных.
  Список слов раскрывается стрелочкой рядом с настройкой: около сорока
  готовых вариантов на русском и английском плюс свои слова и фразы. По
  умолчанию отмечены только звуки запинки («эм», «ээ», «um», «uh») —
  «это», «вот», «короче», «like» и подобные лежат рядом, но выключены,
  потому что это обычные слова и удалять их без спроса значит менять
  сказанное
- Статистика: плитки за сегодня и за всё время, столбцы за 14 дней и
  тепловая карта активности за 17 недель. От тысячи числа сокращаются
  (1,5К, 12К, 1,2М), с округлением вниз
- Приглушение системного звука на время записи — после сигнала начала
  записи, а не до него, иначе сам сигнал оказывался не слышен. Если
  приложение завершилось аварийно во время записи, звук возвращается при
  следующем запуске
...
- Автозапуск при входе в систему — сразу в трей, без окна. Загрузка модели
  начинается после того, как окно отрисовано: сборка ONNX-сессии занимает
  около 4 с и на это время удерживает интерпретатор, так что раньше
  свежеоткрытая панель успевала получить от Windows ярлык «не отвечает»
- Проверка обновлений вручную (настройки, вкладка «Дополнительно») — при
  находке страница релиза открывается в браузере. Фоновая проверка
  включается там же галочкой: раз в 6 часов, о находке сообщает
  уведомление Windows, по клику открывается страница релиза. Об одной и
  той же версии сообщается один раз. Ничего не скачивается и не
  запускается само
...
22 теста на чистой логике: конечный автомат горячих клавиш (включая
проверку, что модификаторы не перехватываются, что сочетание истории не
запускает диктовку и что сочетание правки срабатывает в любом порядке
нажатия), обработка текста и стили, разбор
правок расшифровки на пары замен, сериализация настроек. Микрофон, хук и модель не задействованы, так что
