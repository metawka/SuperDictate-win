...
**Установщик**: [`Dictation-2.3.0-setup.exe`](https://github.com/metawka/SuperDictate-win/releases/latest)
...
| **Alt + Правый Ctrl** | правка последней расшифровки |
...
- **Правка последней расшифровки** (Alt + Правый Ctrl): капсула
  возвращается на экран, и пузырёк предпросмотра над ней становится полем
  ввода с текстом последней диктовки — курсор, стрелки, выделение, клик
  между букв. Enter применяет, Escape закрывает без изменений,
...
разбор правок расшифровки на пары замен, сериализация настроек. Микрофон, хук и модель не задействованы, так что
