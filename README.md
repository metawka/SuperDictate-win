...
**Установщик**: [`Dictation-1.8.0-setup.exe`](https://github.com/metawka/SuperDictate-win/releases/latest)
...
`%LOCALAPPDATA%\D1CT\Models` с индикатором прогресса.
...
- HUD-капсула с индикатором громкости. Цвета капсулы и индикатора
  готовности выбираются вручную, в том числе градиентом
- **Расшифровка на ходу**: над капсулой вторая, поменьше, с текстом по
  ...
  Пузырёк появляется лёгким «раскрытием» и растягивается вместе с фразой, а
  готовый текст держится на экране вместе с капсулой и уходит одновременно
  с ней
...
Всё находится в `%LOCALAPPDATA%\D1CT`. Программа называлась D1CT до 1.8.0;
папка с данными сохранила прежнее имя, чтобы после обновления не пришлось
заново качать модель и не потерялась история.
...
(`auto` / `cuda` / `cpu`) уже есть в интерфейсе; для видеокарты нужно
заменить `onnxruntime` на `onnxruntime-gpu` и поставить CUDA 12 +
cuDNN 9. Код менять не нужно: `auto` сам откатится на CPU, если
